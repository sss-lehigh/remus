...
int main(int argc, char **argv) {
  ...
  if (id >= c0 && id <= cn) {
    ...
    if (id == c0) {
      ...
      compute_threads[0]->cas_root(remus::rdma_ptr<uint64_t>((uint64_t)0),
                                   remus::rdma_ptr<uint64_t>((uint64_t)1));
      ...
      compute_threads[0]->Write<uint64_t>(
          remus::rdma_ptr<uint64_t>(ptr.raw() + offsetof(Counter, value)),
          (uint64_t)0);
      compute_threads[0]->Write<bool>(
          remus::rdma_ptr<bool>(ptr.raw() + offsetof(Counter, locked)), false);
