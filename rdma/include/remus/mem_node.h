...
/// one-sided operations without knowing an rkey, so the only way to communicate
/// an rkey is going to be via a two-sided operation.
///
...
class MemoryNode {
  protected:
  /// A context to associate with a connection
  ///
  ...
  struct IdContext {
    uint32_t machine_id;        // The connected machine's id
    ...
    static inline uint32_t get_machine_id(void *ctx) {
      return reinterpret_cast<IdContext *>(ctx)->machine_id;
    }
  };

  /// SegInfo is a convenience type for associating a MemoryRegistration with
  /// one of the remote-accessible Segments stored in this MemoryNode
  struct SegInfo {
    ...
    while (remaining_conns_ > 0) {
      ...
      if (int ret = rdma_get_cm_event(listen_channel_, &event); ret != 0) {
        if (errno != EAGAIN) {
          ...
          // [mfs] Why yield?  Is there harm in trying again immediately?
          std::this_thread::yield();
          continue;
        }
      }
      // REMUS_DEBUG("  handle_connections() --> Received {} (id={})",
      //             rdma_event_str(event->event), (uintptr_t)(event->id));
  ...
  void on_connect(rdma_cm_id *id, rdma_cm_event *event, ibv_pd *pd) {
    ...
    //
    // [mfs] This config should be double-checked
    auto context = new IdContext{machine_id, {}};
    context->conn_param.private_data = &context->machine_id;
    context->conn_param.private_data_len = sizeof(context->machine_id);
    context->conn_param.rnr_retry_count = 7; // Retry forever
    ...
    ret = rdma_accept(id,
                      machine_id == self_.id ? nullptr : &context->conn_param);
    REMUS_ASSERT(ret == 0, "rdma_accept(): {}",
                 strerror((*__errno_location())));

    // [mfs] We're not checking for errors on this ack?!?
    rdma_ack_cm_event(event);
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    // [mfs]  Should Send have fail-stop semantics instead of returning a status
  ...
  void on_disconnect(rdma_cm_id *id) {
    ...
    //
    // [mfs]  If this ever goes live, be sure to check the return value of
    //        rdma_disconnect()
    rdma_disconnect(id);
...
public:
  /// [mfs] Need to make sure all fields get reclaimed/destructed properly
  ~MemoryNode() = default;

  /// Construct a MemoryNode
  ///
  /// [mfs] The seg size shouldn't be hard-coded.  1MB is certainly more than
  ...
  explicit MemoryNode(const MachineInfo &self,
      ...
      : self_(self), conns_(), send_seg_(1 << 20) {
    ...
    // AUTO-DETECT: Find the device on the experimental network
    std::string dev_name = internal::find_experimental_device(self.address);
    if (dev_name.empty()) {
      REMUS_FATAL("Could not find RDMA device on experimental network");
    }
    
    REMUS_INFO("Node {}: Auto-detected RDMA device '{}' on experimental network",
              id, dev_name);
    ...
    listen_id_ = remus::internal::make_listen_id(self.address, port, dev_name);
    ...
    // }
    REMUS_INFO("Listener thread awaiting {} connections", remaining_conns_);
    // Use the channel that was already created in make_listen_id()
    listen_channel_ = listen_id_->channel;
    
    address_ = self.address;  // You know what you bound to
    port_ = port;             // You know what port you requested
  ...
  /// Don't let the memory node exit until it receives a shut-down signal
  void await_shutdown() {
    ...
    while (!cb_ptr->control_flag_.load()) {
      std::this_thread::yield();
    }
  }
};
