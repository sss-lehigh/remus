...
void metrics(std::string exp_name, uint64_t nnodes, uint64_t nthreads,
             ...
             double read_ratio, double write_ratio, uint64_t think_time, uint64_t elements,
             uint64_t overlap) {
  std::ofstream file(
      "metrics.txt",
  ...
  file << "ThinkTime: " << think_time << std::endl;
...
int main(int argc, char **argv) {
  ...
  uint64_t think_time = args->uget(THINK_TIME);
      ...
      worker_threads.push_back(std::thread([&](int i) {
            ...
            if (id == c0 && i == 0) {
              ...
              metrics(args->sget(EXP_NAME), n_cns, cn_threads, ops, duration,
                      read_ratio, write_ratio, think_time, args->uget(ELEMENTS),
                      overlap);
            } else {
              for (uint64_t j = 0; j < ops; j++) {
                auto ptr = ptrs[j];
