...
auto ARGS = {
  remus::util::BOOL_ARG_OPT("--send_bulk", "If to run test operations multithreaded"),
  remus::util::BOOL_ARG_OPT("--send_test", "If to test the functionality of the methods"),
};
...
int main(int argc, char **argv) {
  ...
  REMUS_ASSERT((bulk_operations ^ test_operations) == 1,
  ...
  if (test_operations) {
    ...
    for (int i = 0; i < scale_size; i++) {
      test_output(show_passing, iht_->contains(pool, i), std::nullopt,
                  std::string("Contains ") + std::to_string(i) + std::string(" false"));
      ...
      test_output(show_passing, iht_->contains(pool, i), std::make_optional(i),
                  std::string("Contains ") + std::to_string(i) + std::string(" true"));
    }
    REMUS_INFO(" = 25% Finished = ");
    for (int i = 0; i < scale_size; i++) {
      test_output(show_passing, iht_->contains(pool, i), std::make_optional(i),
                  std::string("Contains ") + std::to_string(i) + std::string(" maintains true"));
    }
    ...
    for (int i = 0; i < scale_size; i++) {
      test_output(show_passing, iht_->contains(pool, i), std::nullopt,
                  std::string("Contains ") + std::to_string(i) + std::string(" maintains false"));
    }
      ...
      threads.push_back(std::thread(
        [&](int id) {
          ...
          if (id == 0) {
            ...
            REMUS_INFO("Inserts:{}ms Contains:{}ms Total:{}ms", start_to_checkpoint.count(), checkpoint_to_end.count(),
                       total_dur.count());
          }
        },
        t));
