...
inline void __remus_init_log__() {
  ...
  if (!__remus_log__) {
    ...
    __remus_log__ =
        ::spdlog::create_async<::spdlog::sinks::stdout_color_sink_mt>("remus");
#else
    __remus_log__ =
        ::spdlog::create<::spdlog::sinks::stdout_color_sink_mt>("remus");
#endif
  }
  static_assert(REMUS_LOG_LEVEL < spdlog::level::level_enum::n_levels,
                "Invalid logging level.");
  __remus_log__->set_level(
      static_cast<spdlog::level::level_enum>(REMUS_LOG_LEVEL));
