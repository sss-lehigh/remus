...
// placed in a binary search tree (i.e, `remus::ATree`) that augments each node
...
public:
  ...
  remus::util::Status Accumulate(const remus::util::StatusVal<Summary<T>> &other) override;
...
template <typename T> void Summary<T>::UpdatePercentile(double p) {
  ...
  if (node_or.status.t != remus::util::Ok)
    exit(1);
...
template <typename T> remus::util::Status Summary<T>::Accumulate(const remus::util::StatusVal<Summary<T>> &other) {
  ...
  if (d.name_ != name_) {
    return {remus::util::FailedPrecondition, "Unexpected metric ID"};
  }
