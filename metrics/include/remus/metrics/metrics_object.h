...
public:
  ...
  std::string serialize() { return as_json().dump(2); }
...
enum MetricType { Counter, Stopwatch, Summary };
...
public:
  ...
  bool has_summary() { return type == MetricType::Summary; }

  /// If we are a stopwatch metric
  bool has_stopwatch() { return type == MetricType::Stopwatch; }

  /// If we are a counter metric
  bool has_counter() { return type == MetricType::Counter; }
  ...
  void deserialize(std::string data) {
    ...
    name = j["name"];
    if (type != j["type"]) {
      REMUS_WARN("Overriding type in deserialize old={} new={}", MetricType::stringify(type),
                 MetricType::stringify(j["type"]));
    }
    type = j["type"];
    if (type == MetricType::Summary) {
