...
///
template<PolicyType FIRST, PolicyType SECOND, PolicyType THIRD>
class Policy {
public:

  static_assert(FIRST != SECOND and SECOND != THIRD and THIRD != FIRST, 
                "Uniqueness of policies is required");
  ...
  Policy(CompileTimePolicyType<FIRST>, CompileTimePolicyType<SECOND>, CompileTimePolicyType<THIRD>) {
    ...
    static_assert(FIRST == PolicyType::NEXT_CORE_IN_NODE and 
                  SECOND == PolicyType::NEXT_CORE_ACROSS_NODE and 
  ...
  /// @param id thread id
  void pin(int id) const {
    pin_core(cores_map[id % cores_map.size()]);
  }

  auto num_cores() const {
    return cores_map.size();
  }
