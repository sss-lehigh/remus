...
template <typename K, typename V, int N> struct reg_cached_node_pointer {
  ...
  HDS_HOST_DEVICE bool is_deleted() { return atomic_ref<uint64_t>(ptr->deleted).load(memory_order_relaxed) == 1; }

  HDS_HOST_DEVICE void mark_deleted() { atomic_ref<uint64_t>(ptr->deleted).store(1, memory_order_relaxed); }
