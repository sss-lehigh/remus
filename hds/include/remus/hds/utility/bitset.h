...
template <int N> class uint8_bitset {
public:
   ...
   * set bit to x
   */
  template <bool x> HDS_HOST_DEVICE constexpr void set(uint8_t bit, std::integral_constant<bool, x>) {
    if constexpr (x == true) {
      data |= (0x1 << bit);
    } else {
      data &= (~(0x1 << bit));
  ...
  /// get bit x
  HDS_HOST_DEVICE constexpr bool get(uint8_t bit) const { return ((data >> bit) & 0x1) == 0x1; }

  HDS_HOST_DEVICE operator uint8_t() { return data; }
...
template <int N> class uint32_bitset {
public:
  static constexpr int bits = N;
  ...
  HDS_HOST_DEVICE operator uint32_t() { return data; }
