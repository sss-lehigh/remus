...
public:
  ...
  template<typename T>
  HDS_HOST_DEVICE void deallocate(T* ptr, size_t amount) {
    if (!(reinterpret_cast<size_t>(ptr) >= reinterpret_cast<size_t>(memory) 
        and reinterpret_cast<size_t>(ptr) < reinterpret_cast<size_t>(memory + size))) {
