...
template <typename T, int N> struct rdma_node_t {
...
template <typename T, int N> struct rdma_node_pointer;

template <typename T, int N, typename Group> struct rdma_node_reference {
  ...
  HDS_HOST_DEVICE rdma_node_pointer<T, N> next(Group &group) { return rdma_node_pointer<T, N>(local->next_, ctx); }

  template <typename S, int O> friend struct rdma_node_pointer;
...
struct rdma_pointer_constructor {
  ...
  template <typename T, int N> rdma_node_pointer<T, N> operator()(remus::rdma::rdma_ptr<rdma_node_t<T, N>> ptr) {
...
template <typename T, int N> struct rdma_node_pointer {
  ...
  template <typename S, int O, typename Group> friend struct rdma_node_reference;
  ...
  HDS_HOST_DEVICE constexpr bool operator==(const rdma_node_pointer<T, N> &rhs) const { return ptr == rhs.ptr; }
  ...
  template <typename Group> HDS_HOST_DEVICE rdma_node_reference<T, N, std::remove_cvref_t<Group>> load(Group &group) {
