...
public:
  ...
  HDS_HOST_DEVICE explicit constexpr operator T*() const {
    return this->cast(); 
  }
  ...
  HDS_HOST_DEVICE constexpr bool is_marked(int i) const {
    return marks().get(i);
  }
  ...
  HDS_HOST_DEVICE T& operator*() {
    return *this->cast();
  }

  HDS_HOST_DEVICE const T& operator*() const {
    return *this->cast();
  }
  ...
  HDS_HOST_DEVICE constexpr bool operator==(marked_ptr<T, bits> rhs) const {
