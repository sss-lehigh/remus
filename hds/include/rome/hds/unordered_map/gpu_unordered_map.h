...
__global__ void construct_on_gpu(T* ptr, Arg_t ... args) {
  new (ptr) T(args...);
}

template<typename T>
__global__ void destruct_on_gpu(T* ptr) {
  ptr->~T();
}
...
__global__ void remove_um_on_gpu(T* map_, K* keys, bool* results, size_t size) {
  T* map = reinterpret_cast<T*>(__cvta_generic_to_global(map_));
...
template<typename K,
         typename V,
         typename Allocator = allocator::gpu_bump_allocator<allocator::heap_allocator>,
...
public:
  ...
  template<typename A>
  HDS_HOST gpu_unordered_map(uint32_t size, A&& alloc) {
  ...
  HDS_DEVICE auto map() {
    return __cvta_generic_to_global(map_);
  }
