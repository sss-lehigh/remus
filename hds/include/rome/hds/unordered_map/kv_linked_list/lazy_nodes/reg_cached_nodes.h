...
namespace rome::hds::kv_linked_list::lazy_nodes {
    ...
    if(group.is_leader()) {
      ...
      uint32_t xored_bits = static_cast<uint32_t>(bits) ^ 
                            static_cast<uint32_t>(
                              atomic_ref<utility::uint32_bitset<N>>(self.ptr->present).load(memory_order_relaxed)
                            );
...
struct reg_cached_node_pointer {
  ...
  HDS_HOST_DEVICE constexpr explicit operator node*() {
    return ptr;
  }
  ...
  HDS_HOST_DEVICE void set(int offset, K k, V v) {
    ...
    auto tmp = atomic_ref<utility::uint32_bitset<N>>(ptr->present).load(memory_order_acquire);
    tmp.set(offset, true);
    atomic_ref<utility::uint32_bitset<N>>(ptr->present).store(tmp, memory_order_release);
  }
  ...
  template<typename Group>
  HDS_HOST_DEVICE reg_cached_node_pointer next(Group& group) {
  ...
  HDS_HOST_DEVICE void unlock_unsync_leader() {
    atomic_ref<uint64_t>(ptr->lock_).store(0, memory_order_release);
  }   
  ...
  HDS_HOST_DEVICE bool is_deleted() {
    return atomic_ref<uint64_t>(ptr->deleted).load(memory_order_relaxed) == 1;
  }

  HDS_HOST_DEVICE void mark_deleted() {
    atomic_ref<uint64_t>(ptr->deleted).store(1, memory_order_relaxed);
  }

  HDS_HOST_DEVICE bool has_key_at(int idx, K key) {
    return atomic_ref<K>(ptr->keys[idx]).load(memory_order_relaxed) == key;
  }
